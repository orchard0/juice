...
from ._psql import create_octopus_products_db, create_unit_rates_db, create_consumption_db, drop_table, insert_consumption, insert_octopus_energy_products, insert_unit_rates, count, create_calorific_value_db, query_missing_calorific, insert_calorific, insert_updates, query_updates
from ._data import OCOTPUS_API_BASE_URL, PAGE_SIZE, UPDATE_INTERVAL

CONSUMPTION_URL = OCOTPUS_API_BASE_URL + "/{energy_type}-meter-points/{mpan_or_mprn}/meters/{serial_number}/consumption"
TARIFFS_URL = OCOTPUS_API_BASE_URL + "/products/{product_code}/{energy_type}-tariffs/{tariff_code}"
...
def get_octopus_products(psql_config, headers=None):
    ...
    if datetime.now(UTC) < query_updates(
            psql_config, table_name) + timedelta(hours=UPDATE_INTERVAL):
...
def octopus_custom_products_download(psql_config, tariffs, headers=None):
    print('Updating Octopus Products database')
    ...
    for tariff in tariffs:
        ...
        tariff_url = url.format(tariff=tariff)

        r = requests.get(tariff_url, headers=headers)
        data = r.json()

        if len(data.keys()) == 1:
            print(f'{tariff} was not found.', r.status_code)
            continue
...
@staticmethod
def get_calorific_values(psql_config,
                         moved_in_at,
                         LDZ,
                         force_refresh=False,
                         headers=None):
    ...
    if not force_refresh and datetime.now(UTC) < query_updates(
            psql_config, update_name) + timedelta(hours=UPDATE_INTERVAL):
    ...
    url = national_gas_api.format(from_date=from_date,
                                  to_date=to_date,
                                  pub_ids=pub_id)
...
@staticmethod
def get_consumption(psql_config,
                    api_key,
                    energy_type,
                    mpan_or_mprn,
                    serial_number,
    ...
    url = CONSUMPTION_URL.format(
        energy_type=energy_type,
        mpan_or_mprn=mpan_or_mprn,
        serial_number=serial_number) + f'?page_size={PAGE_SIZE}'
...
def get_tariffs(psql_config,
    ...
    for type in ['standard-unit-rates', 'standing-charges']:
        ...
        if not force_refresh and datetime.now(UTC) < query_updates(
                psql_config, table_name) + timedelta(hours=UPDATE_INTERVAL):
            print(
                f"Skipping recently updated {tariff_code} {type.replace('-', ' ')}."
            )
            return
        else:
            print(f"Getting {tariff_code} {type.replace('-', ' ')}.")

        url = TARIFFS_URL.format(
            energy_type=energy_type,
            product_code=tariff_code[5:-2],
            tariff_code=tariff_code) + f'/{type}?page_size={PAGE_SIZE}'
